...
def main():
    ...
    # Third task； adjust CNN: second layer of each VGG blocks with stride 2 and remove max-pooling
    model = CNN_explore_third(in_channels=3, num_classes=10).to(device)
...
VGG_explore_third = [64, 64, 128, 128,  256, 256]  # adjust
class CNN_explore_third(nn.Module):
    def __init__(self, in_channels=3, num_classes=10):
        super(CNN_explore_third, self).__init__()
        self.patchify = nn.Sequential(nn.Conv2d(in_channels=in_channels, out_channels=64,  kernel_size=(2, 2), stride=(2, 2)),  # how to write patchify layer?
                                      nn.BatchNorm2d(64),
                                      nn.ReLU()
                                      )

        self.conv_layers = self.create_conv_layer(VGG_explore_third, in_channels=64)
    ...
    def create_conv_layer(architecture, in_channels):
        ...
        for x in architecture:
            out_channels = x
            if count % 2 == 0:  # 第二个卷积，stride=2实现下采样
                stride = 2
            else:
                stride = 1
