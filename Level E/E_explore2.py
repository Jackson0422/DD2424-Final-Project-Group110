...
def main():
    ...
    # second: update learning rate with different methods

    # Loss function
    criterion = nn.CrossEntropyLoss().to(device)
    optimizer = optim.Adam(model.parameters(), lr=0.001, weight_decay=1e-3)  # L2 regularization # weight decay  # adjust
    ...
    # learning rate warm up + cosine annealing - method 1
    # def get_scheduler(optimize, warmup_epochs, max_epochs):
    #     def lr_lambda(epoch):
    ...
    #         else:
    #             # Cosine Annealing阶段
    #             cosine_progress = (epoch - warmup_epochs) / (max_epochs - warmup_epochs)
    #             return 0.5 * (1 + np.cos(np.pi * cosine_progress))
    #
    #     output = lr_scheduler.LambdaLR(optimize, lr_lambda)
    #     return output
    # scheduler = get_scheduler(optimizer, warmup_epochs=5, max_epochs=30)

    # Step decay - method 2
    # scheduler = lr_scheduler.StepLR(optimizer, step_size=10, gamma=0.1)
    # Cosine Annealing with restart - method 3
