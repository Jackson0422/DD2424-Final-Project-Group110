...
def main():
    ...
    # This is Final explore combination: first task + second task + forth task

    # Forth task； Replace the fully-connected layer after the last convolution layer with a global average pooling layer.
    model = CNN_explore_forth(in_channels=3, num_classes=10).to(device)
...
VGG_explore_forth = [64, 64, 'M', 128, 128, 'M', 256, 256, 'M']
class CNN_explore_forth(nn.Module):
    def __init__(self, in_channels=3, num_classes=10):
        super(CNN_explore_forth, self).__init__()
        self.patchify = nn.Sequential(nn.Conv2d(in_channels=in_channels, out_channels=64,  kernel_size=(2, 2), stride=(2, 2)),  # how to write patchify layer?
                                      nn.BatchNorm2d(64),
                                      nn.ReLU()
                                      )

        self.conv_layers = self.create_conv_layer(VGG_explore_forth, in_channels=64)
